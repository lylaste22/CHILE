...
CCD = sys.argv[1]
dir_in = "/home/student02/dic/"
dir_out1 = "/home/student02/ast1/"
...
def write_txt(asteroid, f):
	...
	for j in xrange(e_num):
            res = str(asteroid[i][j,0]) + " " + str(asteroid[i][j,1]) + " " + str(asteroid[i][j,2]) + " " + str(asteroid[i][j,3]) + " " + str(asteroid[i][j,4]) + " " + str(asteroid[i][j,5]) + " " + str(asteroid[i][j,6]) + "\n"
...
f2 = open(dir_out2+CCD+"asteroids.txt","w")
