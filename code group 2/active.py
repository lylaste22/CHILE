...
def get_parameter(num_feature, info_name, ind_PCA, dir_cat, obj_dict_name):
    ...
    totalLabel = np.zeros(fit.shape[0])
    ...
    return fit, totalLabel, dic, filenames
...
def star_dictionary(num_feature, info_name, ind_PCA, dir_cat, obj_dict_name, star_dict_name, galaxy_dict_name, no_dict_name):
    ...
    fit, totalLabel, dic, filenames = get_parameter(num_feature, info_name, ind_PCA, dir_cat, obj_dict_name)
    ...
    train = pd.DataFrame(dic[totalLabel!=0,:], columns = filenames)
    train.to_pickle(dir_out+train_dict_name)
    
    fit = np.column_stack((fit, totalLabel))
    #column_name_feature = info_name.append('LABEL')
    column_name_feature = ['MAG_ISO','MAG_ISOCOR','MAG_AUTO','MAG_PETRO', 'MAG_BEST', 'MAG_WIN',
             'MAG_PSF', 'MAG_MODEL','MAG_POINTSOURCE', 'MAG_SPHEROID', 'MAG_DISK',
             'MU_EFF_MODEL', 'MU_EFF_SPHEROID', 'MU_EFF_DISK',
             'CHI2_PSF', 'CHI2_MODEL', 'NITER_MODEL', 'NITER_PSF',
             'KRON_RADIUS', 'PETRO_RADIUS',
             ...
             'CLASS_STAR',
             'LABLE']
    print column_name_feature
    fit = pd.DataFrame(fit[totalLabel!=0,:], columns = column_name_feature)
    fit.to_pickle(dir_out+"feature")
