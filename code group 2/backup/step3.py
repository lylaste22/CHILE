...
dir_out = "/home/student02/image/" + name + "/"
...
def drawPlot(CCD, epoch, dir_out, dic):
    ...
    for j in xrange(len(subdic)):
        ...
        sizex = max(20,2.5*data[j,2])
        sizey = max(20,2.5*data[j,2])
        ...
        if subdic[j]!= -1:
            ...
            pylab.savefig(dir_out+CCD+epoch+'_%d.png'%(subdic[j]))
            #plt.show()
